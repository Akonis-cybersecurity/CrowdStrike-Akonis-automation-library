...
ADD_IOC_RESPONSE_OK = {
    "meta": {
        "query_time": 0.283737689,
        "pagination": {"limit": 0, "total": 2},
        "powered_by": "ioc-manager",
        "trace_id": "074ec6b6-c03c-49a9-977b-9147dd881465",
    },
    "errors": None,
    "resources": [
        {
            "id": "4a7730141b08cfb2e6057f94756f5387522b99d7f156193e8a870170c43a43e3",
            "type": "sha256",
            "value": "0451b9c358b1404717f5060aea5711327cf169cd4c5648f5ac23f1a1fb740716",
            "source": "Sekoia.io",
            "action": "prevent",
            "severity": "high",
            "description": (
                "https://app.sekoia.io/intelligence/objects/indicator--8c28aed8-8370-46d5-b7bf-877e6a4840d3"
            ),
            "metadata": {"signed": False, "av_hits": -1},
            "platforms": ["mac", "windows", "linux"],
            "tags": ["Sekoia.io"],
            "expiration": "2024-04-14T00:00:00Z",
            "expired": False,
            "deleted": False,
            "applied_globally": True,
            "from_parent": False,
            "created_on": "2023-04-24T07:26:42.073346862Z",
            "created_by": "c269513e7ece42eaa367c0343c6ec555",
            "modified_on": "2023-04-24T07:26:42.073346862Z",
            "modified_by": "c269513e7ece42eaa367c0343c6ec555",
        },
        {
            "id": "ecd5cab1bd7b380d6c2bdd5811f3ea0bf68f83eb48488f9591604c8dc3e5e57b",
            "type": "md5",
            "value": "ab03a48a29967e738583140e6eb84f0b",
            "source": "Sekoia.io",
            "action": "prevent",
            "severity": "high",
            "description": (
...
def test_get_reference_url():
    action = configured_action(CrowdstrikeActionPushIOCsBlock)
    assert action.get_reference_url(id="test") == "https://app.sekoia.io/intelligence/objects/test"


def test_get_valid_indicators_block():
    action = configured_action(CrowdstrikeActionPushIOCsBlock)

    results = action.get_valid_indicators([STIX_OBJECT_FILE_HASH])
    assert len(results["valid"]) == 2
    assert results["valid"][0].get("action") == "prevent"

    results = action.get_valid_indicators([STIX_OBJECT_IPv4])
    assert len(results["valid"]) == 0  # IPv4 IOCs are not supported on block


def test_get_valid_indicators_detect():
    action = configured_action(CrowdstrikeActionPushIOCsDetect)
    ...
    assert results["valid"][0].get("type") == "ipv4"
    assert results["valid"][0].get("value") == "77.91.78.118"
    assert results["valid"][0].get("action") == "detect"


def test_get_valid_indicators_deduplication():
    action = configured_action(CrowdstrikeActionPushIOCsDetect)
    results = action.get_valid_indicators([STIX_OBJECT_IPv4, STIX_OBJECT_IPv4])
    assert len(results["valid"]) == 1


def test_remove_indicators():
    action = configured_action(CrowdstrikeActionPushIOCsBlock)
    with requests_mock.Mocker() as mock:
        ...
        result = action.remove_indicators([sample_data])
...
def test_run_push_iocs_block():
    def fake_log_cb(message: str, *args):
        print(message)
        return None

    module = CrowdStrikeFalconModule()
    action = CrowdstrikeActionPushIOCsBlock(module=module)
    ...
    action.log_exception = fake_log_cb
...
def test_run_push_iocs_block_revoked():
    def fake_log_cb(message: str, *args):
        print(message)
        return None
    ...
    action.log_exception = fake_log_cb
    revoked_object = STIX_OBJECT_FILE_HASH.copy()
    revoked_object["revoked"] = True
    results = action.run({"stix_objects": [revoked_object]})
    assert results is None
...
def test_run_monitor_ioc():
    def fake_log_cb(message: str, *args):
        print(message)
        return None

    module = CrowdStrikeFalconModule()
    action = CrowdstrikeActionMonitorIOC(module=module)
    # mock the log function of trigger that requires network access to the api for reporting
    action.log = fake_log_cb
    action.module.configuration = {
        "base_url": os.environ["CROWDSTRIKE_BASE_URL"],
    ...
    action.log_exception = fake_log_cb
    results = action.run({"value": "77.91.78.118", "type": "ipv4"})
    assert results is None
...
def test_get_ioc_actions():
    action = configured_action(CrowdstrikeActionGetIOCActions)
    ids = ["action1", "action2"]
    with requests_mock.Mocker() as mock:
        mock.register_uri("POST", "https://my.fake.sekoia/oauth2/token", json=OAUTH2_TOKEN_MOCK)
        mock.register_uri(
            "GET",
            "https://my.fake.sekoia/iocs/entities/actions/v1",
            json={"resources": [{"id": "action1"}, {"id": "action2"}]},
        )
...
def test_get_ioc_actions_empty_ids():
    action = configured_action(CrowdstrikeActionGetIOCActions)
    action.run({"ids": []})
    assert action._error is not None


def test_get_indicators_report():
    action = configured_action(CrowdstrikeActionGetIndicatorsReport)
    with requests_mock.Mocker() as mock:
        mock.register_uri("POST", "https://my.fake.sekoia/oauth2/token", json=OAUTH2_TOKEN_MOCK)
        mock.register_uri(
            "POST",
            "https://my.fake.sekoia/iocs/entities/indicators-report/v1",
            json={"resources": [{"job_id": "123", "status": "pending"}]},
        )

        action.run({"report_format": "csv"})

        assert mock.call_count == 2


def test_get_indicators():
    action = configured_action(CrowdstrikeActionGetIndicators)
    ids = ["ind1", "ind2"]
    with requests_mock.Mocker() as mock:
        mock.register_uri("POST", "https://my.fake.sekoia/oauth2/token", json=OAUTH2_TOKEN_MOCK)
        mock.register_uri(
            "GET",
            "https://my.fake.sekoia/iocs/entities/indicators/v1",
            json={"resources": [{"id": "ind1", "type": "md5"}, {"id": "ind2", "type": "sha256"}]},
        )
...
def test_get_indicators_empty_ids():
    action = configured_action(CrowdstrikeActionGetIndicators)
    action.run({"ids": []})
    assert action._error is not None


def test_create_indicators_action():
    action = configured_action(CrowdstrikeActionCreateIndicators)
    indicators = [{"type": "md5", "value": "abc123", "action": "detect", "platforms": ["windows"]}]
    with requests_mock.Mocker() as mock:
        mock.register_uri("POST", "https://my.fake.sekoia/oauth2/token", json=OAUTH2_TOKEN_MOCK)
        mock.register_uri(
            "POST",
            "https://my.fake.sekoia/iocs/entities/indicators/v1",
            json={"resources": [{"id": "new-ind-1", "type": "md5"}]},
        )

        action.run({"indicators": indicators})

        assert mock.call_count == 2


def test_create_indicators_empty():
    action = configured_action(CrowdstrikeActionCreateIndicators)
    action.run({"indicators": []})
    assert action._error is not None


def test_delete_indicators_action():
    action = configured_action(CrowdstrikeActionDeleteIndicators)
    ids = ["ind1", "ind2"]
    with requests_mock.Mocker() as mock:
        mock.register_uri("POST", "https://my.fake.sekoia/oauth2/token", json=OAUTH2_TOKEN_MOCK)
        mock.register_uri(
...
def test_delete_indicators_empty_ids():
    action = configured_action(CrowdstrikeActionDeleteIndicators)
    action.run({"ids": []})
    assert action._error is not None


def test_update_indicators_action():
    action = configured_action(CrowdstrikeActionUpdateIndicators)
    indicators = [{"id": "ind1", "action": "prevent"}]
    with requests_mock.Mocker() as mock:
        mock.register_uri("POST", "https://my.fake.sekoia/oauth2/token", json=OAUTH2_TOKEN_MOCK)
        mock.register_uri(
            "PATCH",
            "https://my.fake.sekoia/iocs/entities/indicators/v1",
            json={"resources": [{"id": "ind1", "action": "prevent"}]},
        )
...
def test_update_indicators_empty():
    action = configured_action(CrowdstrikeActionUpdateIndicators)
    action.run({"indicators": []})
    assert action._error is not None


def test_search_indicators_action():
    action = configured_action(CrowdstrikeActionSearchIndicators)
    with requests_mock.Mocker() as mock:
        mock.register_uri("POST", "https://my.fake.sekoia/oauth2/token", json=OAUTH2_TOKEN_MOCK)
        mock.register_uri(
            "GET",
            "https://my.fake.sekoia/iocs/queries/indicators/v1",
            json={"resources": ["ind1", "ind2", "ind3"]},
        )

        action.run({"filter": "type:'md5'", "limit": 10})
