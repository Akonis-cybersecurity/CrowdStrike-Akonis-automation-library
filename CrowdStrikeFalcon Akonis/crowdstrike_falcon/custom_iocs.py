...
class CrowdstrikeActionGetIOCActions(CrowdstrikeAction):
    def run(self, arguments):
        ...
        self.log("Retrieving {0} IOC actions".format(len(ids)))
        results = list(self.client.get_ioc_actions(ids))
        return {"actions": results}


class CrowdstrikeActionGetIndicatorsReport(CrowdstrikeAction):
    def run(self, arguments):
        report_format = arguments.get("report_format", "csv")
        search = arguments.get("search")

        self.log("Launching indicators report creation job")
        results = list(self.client.get_indicators_report(report_format=report_format, search=search))
        return {"report": results}


class CrowdstrikeActionGetIndicators(CrowdstrikeAction):
    def run(self, arguments):
        ids: list[str] = arguments.get("ids", [])
        if not ids:
            self.error("List of IDs should not be empty.")
        ...
        self.log("Retrieving {0} indicators".format(len(ids)))
        results = list(self.client.get_indicators(ids))
        return {"indicators": results}


class CrowdstrikeActionCreateIndicators(CrowdstrikeAction):
    def run(self, arguments):
        indicators: list[dict] = arguments.get("indicators", [])
        if not indicators:
            self.error("List of indicators should not be empty.")
            return

        self.log("Creating {0} indicators".format(len(indicators)))
        results = list(self.client.create_indicators(indicators))
        return {"indicators": results}


class CrowdstrikeActionDeleteIndicators(CrowdstrikeAction):
    def run(self, arguments):
        ids: list[str] = arguments.get("ids", [])
        if not ids:
            self.error("List of IDs should not be empty.")
        ...
        self.log("Deleting {0} indicators".format(len(ids)))
        list(self.client.delete_indicators(ids))
        self.log("Indicators deleted.")


class CrowdstrikeActionUpdateIndicators(CrowdstrikeAction):
    def run(self, arguments):
        indicators: list[dict] = arguments.get("indicators", [])
        if not indicators:
            self.error("List of indicators should not be empty.")
        ...
        self.log("Updating {0} indicators".format(len(indicators)))
        results = list(self.client.update_indicators(indicators))
        return {"indicators": results}


class CrowdstrikeActionSearchIndicators(CrowdstrikeAction):
    def run(self, arguments):
        ...
        self.log("Searching indicators")
        results = list(self.client.search_indicators(filter=fql_filter, sort=sort, limit=limit, offset=offset))
        return {"indicator_ids": results}
